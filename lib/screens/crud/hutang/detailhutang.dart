...
class Detailhutang extends StatefulWidget implements PreferredSizeWidget {
...
class _DetailhutangState extends State<Detailhutang> {
  ...
  var no_telepon;
  var status_hutang;
  var jumlah_hutang_dibayar;
  var tanggal_hutang_dibayar;
  ...
  Future<void> _getHutangById() async {
    final response = await Network().getData('/hutang?id='+widget.id.toString());
    indexdata = json.
    decode(response.body)['data']['data_hutang'];
    setState(() {
      id_hutang = indexdata['id'];
      nama_hutang = indexdata['nama_hutang'];
      deksripsi = indexdata['deksripsi'];
      jumlah_hutang =  indexdata['jumlah_hutang'].toString();
      tanggal_hutang = indexdata['tanggal_hutang'].toString();
      jumlah_hutang_dibayar =  indexdata['jumlah_hutang_dibayar'].toString();
      if(indexdata['status_hutang'] == 1){
        status_hutang = "Lunas";
      }else{
        status_hutang = "Belum Lunas";
      }
      no_telepon = indexdata['no_telepon'];
      tanggal_hutang_dibayar = indexdata['tanggal_hutang_dibayar'].toString();
    });
  }
  void _deleteHutang() async {
    var data = null;
    var res = await Network().postData(data, '/hutang/destroy/'+id_hutang.toString());
    print(res.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      drawer: FlutterMoneyquDrawer(currentPage: "detail-hutang"),
      body:  Stack(
        children: [
          Column(
            children: [
      ...
      floatingActionButton: new FloatingActionButton(
        ...
        onPressed:(){ Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => Ubahhutang(id: id_hutang)),
        );},
        tooltip: 'Increment',
